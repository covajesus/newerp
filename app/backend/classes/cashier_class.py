...
class CashierClass:
    ...
    def get_all(self, branch_office_id = ''):
        try:
            if branch_office_id == '':
                data = self.db.query(CashierModel).filter(CashierModel.folio_segmennt_id != 9).order_by(CashierModel.cashier).all()
            else :
                data = self.db.query(CashierModel).filter(CashierModel.branch_office_id == branch_office_id).filter(CashierModel.folio_segmennt_id != 9).order_by(CashierModel.cashier).all()
            if not data:
