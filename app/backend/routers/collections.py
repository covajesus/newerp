...
def total_collections(session_user: UserLogin = Depends(get_current_active_user), db: Session = Depends(get_db)):
    data = CollectionClass(db).total_collections(session_user.rol_id)
    return {"message": data}
