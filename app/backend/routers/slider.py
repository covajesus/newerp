...
from app.backend.schemas import Employee, UpdateEmployee, SearchEmployee, UserLogin, EmployeeList, UploadSignature, UploadPicture, ExternalApiCredentials
...
def get_resources(credentials: ExternalApiCredentials, db: Session = Depends(get_db)):
    """
    Obtiene recursos desde la API externa usando el token externo
    """
    data = SliderClass(db).get_resources(credentials.external_token, credentials.rut, credentials.password)
    return {"message": data}
